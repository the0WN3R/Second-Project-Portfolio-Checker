...
with open("portfolio.txt") as file:
    previous_portfolio_value = float(file.readline().strip())
...
with open("portfolio.txt", "w") as file2:
    ...
    for price in latest_prices:
        file2.write(str(latest_prices[price]) + "\n")
...
if detailed_look in ["y", "Y", "yes", "Yes"]:
    ...
    if detailed_ticker not in stocks:
        ...
        try:
            ...
            with open("portfolio.txt") as file3:   
                if detailed_ticker == "ADBE":
                    detailed_stock_change = ADBE_price - float(file3.readline()[1].strip())
                elif detailed_ticker == "BA":
                    detailed_stock_change = BA_price - float(file3.readline()[2].strip())
                elif detailed_ticker == "DELL":
                    detailed_stock_change = DELL_price - float(file3.readline()[3].strip())
                elif detailed_ticker == "DIA":
                    detailed_stock_change = DIA_price - float(file3.readline()[4].strip())
                elif detailed_ticker == "DLTR":
                    detailed_stock_change = DLTR_price - float(file3.readline()[5].strip())
                elif detailed_ticker == "LLY":
                    detailed_stock_change = LLY_price - float(file3.readline()[6].strip())
                elif detailed_ticker == "MSFT":
                    detailed_stock_change = MSFT_price - float(file3.readline()[7].strip())
                elif detailed_ticker == "QQQ":
                    detailed_stock_change = QQQ_price - float(file3.readline()[8].strip())
                elif detailed_ticker == "SPY":
                    detailed_stock_change = SPY_price - float(file3.readline()[9].strip())
                elif detailed_ticker == "TMUS":
                    detailed_stock_change = TMUS_price - float(file3.readline()[10].strip())
                else:
                    detailed_stock_change = YETI_price - float(file3.readline()[11].strip())
            ...
            print(f"Your net gain/loss for {detailed_ticker} today was: ${round(detailed_stock_change, 2)}")
        except Exception:
            print(f"I'm sorry, but something went wrong and we could not find any data for {detailed_ticker}.")
